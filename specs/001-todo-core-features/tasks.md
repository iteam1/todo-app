...
- [ ] T021 [P] [US1] Implement `src/components/TodoItem.tsx` — `<li data-todo-id={todo.id}>` with `<input type="checkbox" disabled aria-label="Mark '${todo.text}' as complete" checked={false} />` (toggle wired in T027/US2; `disabled` stub avoids unhandled click), task text display; min `h-11 w-11` touch targets; no strikethrough styling yet (added in T027)
...
- [ ] T040 [US4] Update `src/components/TodoList.tsx` — pass `onDelete` prop through to each `<TodoItem>` (note: `data-todo-id` is already on the `<li>` in `TodoItem` from T021; no wrapper `<li>` needed here)
